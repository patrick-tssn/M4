...
from longva_backend_dup import LongVA
...
class OmniChatServer(object):
    def __init__(self, ip='0.0.0.0', port=60808, run_app=True,
        ...
        self.client = LongVA(pretrained=ckpt_dir, model_name="llava_qwen", device_map=device, attn_implementation="eager")
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as audio_f, \
                ...
                if img_path is not None:
                    resp_generator = self.client.output(audio_f.name,img_f.name,
    ...
    def generator(self, resp_generator):
        ...
        for text_stream in resp_generator:
            # yield b'\r\n--frame\r\n'
