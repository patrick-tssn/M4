...
class LongVA:
    ...
    def output(self, requests: dict, gen_kwargs: dict):
        ...
        with torch.inference_mode():
            ...
            # get the prob of visual tokens
            input_length = input_ids.shape[1]
            img_idx = input_ids[0].tolist().index(-200)
            img_logits = logits[:, img_idx:img_idx-input_length+1]
            img_prob = torch.softmax(img_logits, dim=-1)
            img_prob_n = img_prob[:,:,198]
            b, l = img_prob_n.shape
            img_prob_n = img_prob_n.reshape(b, l//144, 144).mean(2)
            print(img_prob_n)
            token_point = torch.argmax(img_prob_n, dim=-1)
            # frame_point = int(token_point / img_prob.shape[1] * 31)
            # assert img_prob.shape[1] % 144 == 0
            print("frame position: ", token_point)
...
if __name__ == "__main__":
    ...
    # model = LongVA(pretrained="checkpoints/longva7b-llavanextsub10k-qwen2-stop", model_name="llava_qwen", device_map=args.device)
    ...
    model = LongVA(pretrained="checkpoints/longva7b-llavanextsub10k-llama31-rev", conv_template="llava_llama_3", model_name="llava_llama3", device_map=args.device)
    # if args.image_path:
