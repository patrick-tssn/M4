...
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-llavanext-vst-vt-st-mix-300k.out
#SBATCH --error=./slurm_logs/finetune-longva-llavanext-vst-vt-st-300k.error.out
...
LLM_VERSION="checkpoints/longva7b-qwen2-llavanext-speech-full-start-filter/checkpoint-2500"
...
MID_RUN_NAME="longva7b-qwen2-llavanext-vst-vt-st-300k"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava_next-vst-vt-st-300k.json \
