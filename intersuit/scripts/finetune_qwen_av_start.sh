...
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-llavanext-speech-full-start-filter.out
#SBATCH --error=./slurm_logs/finetune-longva-llavanext-speech-full-start-filter.error.out
...
MID_RUN_NAME="longva7b-qwen2-llavanext-speech-full-start-filter"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava_next-speech-start-filter.json \
