#!/bin/bash
#SBATCH --job-name=ivaspeechLVsub
#SBATCH --partition=HGX,DGX
#SBATCH --exclude=dgx-hyperplane[13],hgx-hyperplane[00]
...
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-voiceassistant-special.out
#SBATCH --error=./slurm_logs/finetune-longva-voiceassistant-special.error.out
...
# LLM_VERSION="checkpoints/longva7b-qwen2-llavanext-speech-full-filter-special"
...
MID_RUN_NAME="longva7b-qwen2-llavanext-speech-voiceassistant-special"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava_next-st-10k-special.json \
    --image_folder inputs/images/llava-next \
    --speech_folder inputs/speech \
