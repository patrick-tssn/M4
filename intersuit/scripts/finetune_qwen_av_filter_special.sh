...
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-llavanext-speech-full-filter-special.out
#SBATCH --error=./slurm_logs/finetune-longva-llavanext-speech-full-filter-special.error.out
...
LLM_VERSION="checkpoints/longva7b-qwen2-llavanext-speech-full-filter/checkpoint-3000"
...
MID_RUN_NAME="longva7b-qwen2-llavanext-speech-full-filter-special"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava_next-speech-100-special.json \
    ...
    --group_by_modality_length_mm True \
    --is_mop True \
