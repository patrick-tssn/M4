...
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-llavanext-speech-cosyvoice.out
#SBATCH --error=./slurm_logs/finetune-longva-llavanext-speech-cosyvoice.error.out
...
MID_RUN_NAME="longva7b-llavanext-qwen2-speech-cosyvoice"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava_next-speech-100.json \
