...
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-sub10k-ORNS-speech-all.out
#SBATCH --error=./slurm_logs/finetune-longva-sub10k-ORNS-speech-all.error.out
...
LLM_VERSION="checkpoints/longva7b-qwen2-voiceassistant-100k/checkpoint-1500"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava-next-sub-10k-ORNS1111-speech-0-ns.json \
