...
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-longva-sub10k-RNS.out
#SBATCH --error=./slurm_logs/finetune-longva-sub10k-RNS.error.out
...
MID_RUN_NAME="longva7b-llavanextsub30k-qwen2-RNS"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --master_port="${PORT}" \
    ...
    --data_path inputs/texts/llava-next-sub-30k-RNS.json \
